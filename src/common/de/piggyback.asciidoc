...
*Hinweis:* Huckepack-Dateien von Hosts, deren Namen mit einem Punkt beginnen, werden nicht in {CMK} verarbeitet.
Dies betrifft z.B. Namen wie ".", ".hostname", and ".hostname.domain.com".
Um diese Hosts ins Monitoring aufzunehmen, müssen die Host-Namen wie beschrieben geändert werden.
...
image::piggyback_processing_rule.png[alt="Eingabe der Regeln für die Piggyback-Verarbeitung."]

Als erstes geben Sie, wie bei jeder Regel, die Basisinformationen ein, hier vor allem einen eindeutigen Titel oder eine kurze Beschreibung.
Im Abschnitt [.guihint]#Processing of piggybacked host data# folgen dann die eigentlich interessanten Angaben zur Verarbeitung der Piggyback-Daten.

image::piggyback_processing_rule2.png[alt="Festlegung der Regeln für veraltete Piggyback-Daten."]

Der xref:dcd#[DCD] erleichtert Ihnen die Arbeit bei der Verwaltung der Piggyback-Daten.
So entfernt er unter anderem automatisch alle Hosts/Objekte, zu denen keine Piggyback-Daten (mehr) von einem Quell-Host geliefert werden.
Mit der Option [.guihint]#Keep hosts while piggyback source sends piggyback data# legen Sie fest, nach welcher Zeitspanne der DCD mit dem Entfernen starten soll.
...
Nach Ablauf der definierten Zeitspanne werden die Daten als xref:monitoring_basics#stale[_stale_] angezeigt.
Außerdem legen Sie den Zustand des Services Check_MK fest.
...
Nachdem Sie die Behandlung der Piggyback-Daten im Allgemeinen festgelegt haben, können Sie unter [.guihint]#Exceptions for piggybacked hosts# mit den beschriebenen Optionen gezielt für einzelne Hosts eine gesonderte Behandlung definieren. 
...
Beachten Sie, dass in xref:distributed_monitoring#[verteilten Umgebungen] aktuell Quell-Host und die Piggyback-Hosts in der gleichen Instanz überwacht werden müssen.
Dies liegt daran, dass die Übertragung der Daten zwischen den Hosts aus Effizienzgründen mit einem lokalen Dateiaustausch über das Verzeichnis `~/tmp/check_mk` läuft.

// Zukünftige Versionen von {CMK} werden eventuell einen Mechanismus anbieten, der optional den Austausch von Piggyback-Daten über Instanzgrenzen hinweg ermöglicht.
